...
- **Purpose:** organise products into a (possibly nested) category tree used by storefront nav and filters.
- **Layout:** two panes — category tree (left, ~320px, drag-reorder + nest) | category detail (right): name, slug, parent select, visibility toggle, product count, SEO blurb; sticky save bar.
- **Visible components:** tree rows (drag handle, expand chevron, name, count), "+ New category", detail form, assigned-products list.
- **Actions:** create, rename, set parent, reorder, nest/unnest, toggle visibility, delete, save.
- **States:** selected, expanded/collapsed, hidden, drag.
- **Interactions:** drag to reorder/nest; click → detail; delete asks confirm (warns if products attached).
- **Responsive / Loading / Empty / Error / Success / Hover / Focus / Disabled:** mirror S2 patterns (list+editor+sticky save, confirm modal for destructive, skeleton rows, empty state "No categories yet").
...
- **Layout:** list/table view (searchable, filterable) → customer detail (split): profile card, LTV/order stats, orders table, notes.
- **Visible components:** filter bar + search, customer rows (avatar monogram, name, email, orders, LTV, last order), detail panel reusing Order-detail timeline/cards.
- **Actions:** search/filter, open customer, open an order, add note, (optional) block/flag.
- **States:** selected, VIP/new/flagged chips, sortable columns.
- **Interactions:** row click → detail; sort columns; pagination.
- **Responsive / Loading / Empty / Error / Success / Hover / Focus / Disabled:** standard table patterns (skeleton rows, "No customers match" empty, hover row tint, focusable rows, pagination disabled at bounds).

---

## S11 · Homepage Editor 🟡 (header button, distinct mode)

- **Purpose:** compose the storefront homepage (hero, featured sections, banners) — opened from the **header**, not the sidebar.
- **Layout:** full-screen editor mode — left **section/block list** (add, reorder, toggle), center **live homepage canvas preview**, right **block settings panel**; top bar with Exit, device toggle (desktop/mobile), Save/Publish.
- **Visible components:** block list (Hero, Trending now [bound to S3 Featured], Categories, Banner, Footer), drag handles, per-block settings (text, image dropzone, links, visibility), live preview, Publish.
- **Actions:** add/remove/reorder blocks, edit block, toggle device preview, save draft, publish.
- **States:** editing mode, block selected, draft vs published, device = desktop/mobile.
- **Interactions:** drag blocks; edits reflect in preview live; publish pushes live (env badge).
...
## S12 · Developer Tools 🟡 (Settings sub-page)

- **Purpose:** API keys, webhooks, supplier API connection, logs, environment.
- **Layout:** settings sub-nav | content cards: API keys (table with reveal/rotate/revoke), Webhooks (endpoint list + test + delivery log), Supplier API (connection status, credentials, sync now), Environment (LIVE/TEST switch), Event log.
- **Visible components:** key rows (masked value, copy, rotate, revoke), webhook rows (URL, events, status, last delivery), supplier status card (connected/error), log viewer (mono).
- **Actions:** create/rotate/revoke key, add/edit/test webhook, connect/sync supplier, switch environment, copy values.
- **States:** key active/revoked, webhook healthy/failing, supplier connected/error/syncing, env LIVE/TEST.
- **Interactions:** copy → toast; rotate/revoke → confirm modal; test webhook → inline result; sync → progress + result.
- **Responsive / Loading / Empty / Error / Success / Hover / Focus / Disabled:** tables with skeleton rows; empty "No API keys yet"; supplier error reuses the error-card component; destructive actions gated by confirm modal; copy/reveal buttons focusable.

---

### Cross-screen note
S9–S12 are **specified, not yet drawn**. They reuse existing primitives (list+editor+sticky save, split detail, grouped table, settings sub-nav, confirm modal, toast, status badges) so they slot into the same shell without new visual language.
