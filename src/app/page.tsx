...
// Below-the-fold trust sections are code-split so they never weigh on the
// initial homepage payload (they still render on the server for SEO).
const CustomerReviews = dynamic(() => import("@/components/trust/CustomerReviews"));
const AcceptedPayments = dynamic(() => import("@/components/trust/AcceptedPayments"));
const FaqSection = dynamic(() => import("@/components/trust/FaqSection"));
  ...
  return (
    <div className="container-page pb-14 sm:pb-0">
      ...
      {settings.homepage.showWhyGhost && (
        <WhyGhost heading={settings.homepage.whyChooseUsTitle} />
      )}

      {/* Premium 5-step delivery flow. Keeps the #how-it-works anchor that the
          hero's secondary CTA links to. */}
      {settings.homepage.showHowItWorks && (
        <DeliverySteps
          id="how-it-works"
          heading={settings.homepage.howItWorksTitle}
          subheading={settings.homepage.howItWorksSubtitle}
        />
      )}

      {settings.homepage.showReviews && <CustomerReviews />}

      {settings.homepage.showPaymentMethods && <AcceptedPayments />}

      {settings.homepage.showFaq && <FaqSection />}
