/**
 * The ghost.ma activation-guide library — the platforms we sell (Steam,
 * PlayStation, Xbox, Nintendo, Valorant) plus the popular platforms Moroccan
 * customers commonly buy (Netflix, Spotify, Roblox, Fortnite, Google Play,
 * Apple, Amazon, Discord, Razer Gold, Twitch, PUBG Mobile, Free Fire, League of
 * Legends, Minecraft).
 *
 * The copy is ORIGINAL (written for ghost.ma, not copied from any third-party
 * source) and conveys each platform's standard, factual redemption flow. This is
 * the single source of truth used by BOTH the admin "populate guides" action
...
export interface ActivationGuideSpec {
  ...
  /** Search aliases; also the keywords used to match products + categories. */
  aliases: string[];
  intro: string;
  stepsTitle?: string;
  steps: string[];
  ...
  faq: [string, string][];
  tip: { title: string; message: string; type: NavigatorTipType; ctaLabel: string; ctaUrl: string };
}

export const ACTIVATION_GUIDE_SPECS: ActivationGuideSpec[] = [
...
/** Build the typed content blocks for a guide (stable per-slug block ids). */
export function buildActivationBlocks(s: ActivationGuideSpec): GuideBlock[] {
...
export function buildActivationFaq(s: ActivationGuideSpec) {
  return s.faq.map(([question, answer], i) => ({ id: `${s.slug}-faq-${i + 1}`, question, answer }));
}
