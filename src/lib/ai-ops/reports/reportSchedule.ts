/**
 * Daily Reports scheduling — the PURE cron + timezone "is this report due?"
 * logic (no server-only, no DB, no provider).
 *
 * The base AI scheduler (src/lib/ai-ops/scheduler.ts) only guarantees "run once
 * per day/hour/week bucket" — it does NOT gate on the wall-clock time of day, so
 * it cannot express "08:00 Africa/Casablanca". Executive reports need real
 * time-of-day scheduling in a configured timezone, so this module adds a small,
 * focused cron evaluator and combines it with the base lock/idempotency rules.
 *
 * Kept pure (timestamp + timezone passed in) so the whole due-decision is
 * unit-testable without a clock, a DB, or ICU surprises.
 */
 ...
 * the report as due once the scheduled minute-of-the-hour has passed; the
 * idempotency bucket (below) then dedupes the repeated ticks within that hour.
 *
 ...
 * null if none within a year / the cron is malformed. Used only for the admin
