...
function WhatsappCard({ order }: { order: AdminOrderDTO }) {
  const w = order.whatsapp;
  const hasPhone = !!w.waMeUrl;
  const green = "#25D366";

  return (
    <div style={{ ...cardStyle, padding: 16 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
        <svg width="15" height="15" viewBox="0 0 24 24" fill={green} aria-hidden>
        ...
        <div style={{ fontSize: 13, fontWeight: 600, color: C.text }}>WhatsApp</div>
      </div>

      <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "baseline" }}>
        <span style={{ fontSize: 11.5, color: C.faint }}>Téléphone</span>
        <span style={{ fontSize: 12.5, color: hasPhone ? C.text : C.faint, fontWeight: 500, textAlign: "right" }}>
          {w.phone || "aucun numéro"}
        </span>
      </div>

      {/* Manual delivery: admin copies the message or opens WhatsApp with it
          pre-filled. Automatic send is not wired yet — same message will feed it. */}
      <div style={{ marginTop: 14, display: "flex", flexDirection: "column", gap: 8 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
          <span style={{ fontSize: 11.5, color: C.faint }}>Message prêt à envoyer</span>
          <CopyButton value={w.message} label="Copier" />
        </div>
        <textarea
          readOnly
          value={w.message}
        ...
        {hasPhone ? (
          ...
          <p style={{ fontSize: 11.5, color: C.faint, margin: 0 }}>
            Aucun numéro de téléphone sur le compte — copiez le message et envoyez-le manuellement.
          </p>
        )}
        <p style={{ fontSize: 11, color: C.faint, margin: "2px 0 0" }}>
          Envoi automatique bientôt disponible.
        </p>
      </div>
    </div>
  );
