...
export default function CommandSearch({
  ...
  /**
   * Optional custom trigger. Receives the palette opener and its open state so
   * the host (AdminShell) can supply the topbar's own search-bar design on
   * desktop and a compact icon button on mobile, while this component keeps
   * ownership of the single ⌘K listener and the overlay. When omitted, the
   * default trigger pill is rendered.
   */
  renderTrigger?: (open: () => void, opened: boolean) => ReactNode;
} = {}) {
  ...
  return (
    <>
      ...
      {renderTrigger ? (
        renderTrigger(openPalette, open)
      ) : (
        <button
          ref={triggerRef}
          type="button"
