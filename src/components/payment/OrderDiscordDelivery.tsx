"use client";

import { useEffect, useState } from "react";
import {
  getOrderDiscordContextAction,
  setOrderDiscordDeliveryAction,
  type OrderDiscordState,
} from "@/app/actions/discord";
import Checkbox from "@/components/ui/Checkbox";

function DiscordGlyph({ className = "h-4 w-4" }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" className={className} fill="#5865F2" aria-hidden>
      <path d="M20.317 4.369a19.79 19.79 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.865-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028c.462-.63.874-1.295 1.226-1.994a.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128c.126-.094.252-.192.372-.291a.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.009c.12.099.246.198.373.292a.077.077 0 0 1-.006.127 12.3 12.3 0 0 1-1.873.891.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.055c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.028zM8.02 15.331c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.211 0 2.176 1.096 2.157 2.42 0 1.333-.955 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.211 0 2.176 1.096 2.157 2.42 0 1.333-.946 2.418-2.157 2.418z" />
    </svg>
  );
}

/**
 * Compact per-order Discord DM delivery control on the payment page. Renders a
 * different affordance per connection state; only an activated owner sees the
 * actual checkbox. Discord is purely additive and never blocks payment.
 */
export default function OrderDiscordDelivery({ orderId }: { orderId: string }) {
  const [state, setState] = useState<OrderDiscordState | null>(null);
  const [owner, setOwner] = useState(false);
  const [checked, setChecked] = useState(false);
  const [saving, setSaving] = useState(false);

  useEffect(() => {
    let active = true;
    getOrderDiscordContextAction(orderId)
      .then((ctx) => {
        if (!active || !ctx) return;
        setState(ctx.state);
        setOwner(ctx.owner);
        setChecked(ctx.requested);
      })
      .catch(() => {
        /* Discord is optional — silently skip on failure. */
      });
    return () => {
      active = false;
    };
  }, [orderId]);

  if (!state) return null;

  async function onToggle(next: boolean) {
    setChecked(next);
    setSaving(true);
    const res = await setOrderDiscordDeliveryAction(orderId, next);
    setSaving(false);
    if (!res.ok) setChecked(!next);
  }

  const shell =
    "rounded-2xl border border-[rgba(88,101,242,0.22)] bg-[rgba(88,101,242,0.06)] px-[18px] py-4";

  // Activated owner: the real preference checkbox.
  if (state === "activated" && owner) {
    return (
      <div className={shell}>
        <div className="mb-2 flex items-center gap-2">
          <DiscordGlyph />
          <h2 className="text-sm font-semibold text-white">Livraison Discord</h2>
        </div>
        <Checkbox
          checked={checked}
          disabled={saving}
          onChange={(e) => onToggle((e.target as HTMLInputElement).checked)}
          label={
            <span className="text-[13px] text-text">
              Recevoir aussi cette commande par Discord
            </span>
          }
        />
        <p className="mt-2 pl-[26px] text-xs leading-relaxed text-[#9A9FAB]">
          Votre code sera également envoyé en message privé sur Discord après validation du
          paiement et livraison de la commande.
        </p>
      </div>
    );
  }

  // Connected but not activated: prompt to activate.
  if (state === "connected_not_activated") {
    return (
      <div className={shell}>
        <div className="mb-1 flex items-center gap-2">
          <DiscordGlyph />
          <h2 className="text-sm font-semibold text-white">Recevez votre code sur Discord</h2>
        </div>
        <p className="text-xs leading-relaxed text-[#9A9FAB]">
          Activez les messages Discord pour utiliser cette option.
        </p>
        <a href="/account" className="btn-ghost mt-3 inline-flex text-[13px]">
          Activer Discord
        </a>
      </div>
    );
  }

  // Not connected (logged in): prompt to connect.
  if (state === "not_connected") {
    return (
      <div className={shell}>
        <div className="mb-1 flex items-center gap-2">
          <DiscordGlyph />
          <h2 className="text-sm font-semibold text-white">Recevez votre code sur Discord</h2>
        </div>
        <p className="text-xs leading-relaxed text-[#9A9FAB]">
          Connectez Discord pour activer cette option.
        </p>
        <a href="/auth/discord?mode=link" className="btn-ghost mt-3 inline-flex text-[13px]">
          Connecter Discord
        </a>
      </div>
    );
  }

  // Guest: gentle account CTA, never a username field.
  if (state === "guest") {
    return (
      <div className={shell}>
        <div className="mb-1 flex items-center gap-2">
          <DiscordGlyph />
          <h2 className="text-sm font-semibold text-white">Recevez votre code sur Discord</h2>
        </div>
        <p className="text-xs leading-relaxed text-[#9A9FAB]">
          Connectez-vous et liez Discord pour recevoir vos commandes en message privé.
        </p>
        <a href="/login" className="btn-ghost mt-3 inline-flex text-[13px]">
          Se connecter
        </a>
      </div>
    );
  }

  return null;
}
