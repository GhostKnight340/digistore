import type { NavigatorTipSetting, NavigatorTipType } from "@/lib/trust/content";

/**
 * A single Navigator tip card: a dark bordered card with a subtle blue inset
 * glow (the HeroDeliveryCard idiom) and a small Navigator mascot carrying a
 * coded type badge. Mirrors the category `NavigatorTip` visual language but is
 * self-contained and driven by the reusable `NavigatorTipSetting` model so it
 * can be used anywhere (homepage, product, collection, campaign pages).
 *
 * The mascot art is decorative (`alt=""`); the badge type is announced only to
 * screen readers so they aren't given a duplicate mascot description.
 */

const BADGE: Record<
  NavigatorTipType,
  { color: string; label: string; glyph: React.ReactNode }
> = {
  information: {
    color: "#3e7bfa",
    label: "Information",
...
export default function NavigatorTipCard({ tip }: { tip: NavigatorTipSetting }) {
    ...
    <div className="relative overflow-hidden rounded-[18px] border border-border bg-[linear-gradient(158deg,#1d2638_0%,#141a27_52%,#0d1017_100%)] p-5 shadow-soft sm:p-6">
      ...
      <div className="relative flex items-start gap-4">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-1.5 text-[14px] leading-relaxed text-muted">{tip.message}</p>
        </div>
      </div>
    </div>
  );
