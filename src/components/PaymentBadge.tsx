 ...
 * method; only the brand tint, label treatment and mark change.
 *
...
function badgeLabel(key: BrandKey | null, display: ResolvedPaymentDisplay): ReactNode {
  if (key === "paypal") {
    return (
      <span className="text-[15px] font-bold italic tracking-[0.005em]">
  ...
  if (key === "usdt") {
    return (
      <span className="flex items-baseline gap-1.5">
        <span className="text-[15px] font-semibold tracking-[0.005em]" style={{ color: "#9BE9CC" }}>
          USDT
        </span>
        <span className="font-mono text-[12px] font-medium" style={{ color: "#6FCDA9" }}>
          TRC-20
        </span>
      </span>
    );
  ...
  return (
    <span className="text-[15px] font-semibold tracking-[0.005em]">{display.displayName}</span>
  );
}

/** Tile mark: admin logo when uploaded, else the handoff's line glyphs
 *  (bank/card), italic PayPal "P", or the method initials. */
function tileMark(key: BrandKey | null, display: ResolvedPaymentDisplay) {
  ...
  if (key === "bank") {
    return (
      <svg viewBox="0 0 24 24" fill="none" stroke="#9CC0FF" strokeWidth={1.8} strokeLinecap="round" strokeLinejoin="round" className="h-[22px] w-[22px]" aria-hidden>
  ...
  if (key === "card") {
    return (
      <svg viewBox="0 0 24 24" fill="none" stroke="#CFC0FB" strokeWidth={1.8} strokeLinecap="round" strokeLinejoin="round" className="h-[23px] w-[23px]" aria-hidden>
  ...
  if (key === "paypal") {
    return (
      <span className="text-[24px] font-extrabold italic leading-none" style={{ color: "#8FBEFF" }}>
        P
      </span>
    );
  }
  return <span className="text-[13px] font-bold">{display.initials}</span>;
}
...
export default function PaymentBadge({
  ...
  return (
    <div
      className={`group flex h-16 items-center gap-3.5 rounded-2xl border px-5 transition-all duration-[.18s] ease-[ease] [background:linear-gradient(180deg,rgb(var(--pb)/0.10),rgb(var(--pb)/0.05))] [border-color:rgb(var(--pb)/0.25)] [box-shadow:0_4px_18px_rgba(0,0,0,0.30),inset_0_1px_0_rgba(255,255,255,0.05)] hover:-translate-y-px hover:[border-color:rgb(var(--pb)/0.52)] hover:[box-shadow:0_8px_26px_rgba(0,0,0,0.40),0_0_0_1px_rgb(var(--pb)/0.19),inset_0_1px_0_rgba(255,255,255,0.08)] ${className}`}
      style={vars}
    >
      <span className="flex h-[42px] w-[42px] shrink-0 items-center justify-center rounded-xl border [background:linear-gradient(155deg,rgb(var(--pb)/0.30),rgb(var(--pb)/0.12))] [border-color:rgb(var(--pb)/0.40)] [box-shadow:inset_0_1px_0_rgba(255,255,255,0.16),0_2px_6px_rgba(0,0,0,0.32)]">
        {tileMark(key, display)}
      </span>
      {badgeLabel(key, display)}
    </div>
  );
}
